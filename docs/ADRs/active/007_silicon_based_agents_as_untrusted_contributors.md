...
Claude Code is currently the primary silicon-based agent tool used in this project.
...
In a system where silent statistical errors or geographic misalignment can propagate into humanitarian decisions, these properties represent elevated risk. Without explicit guardrails, silicon-based agents introduce architectural, semantic, and safety risks that are difficult to detect post hoc.
...
(see `docs/contributor_protocols/silicon_based_agents.md`).
...
- LLM-based coding assistants (including Claude Code)
...
- carbon-based agents (see `docs/contributor_protocols/carbon_based_agents.md`)
...
- Silicon-based agent-assisted changes must comply with the Silicon-Based Agent Protocol (`docs/contributor_protocols/silicon_based_agents.md`)
...
(`docs/contributor_protocols/silicon_based_agents.md`), which may evolve

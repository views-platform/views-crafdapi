...
This is especially dangerous in views-faoapi, which:
- delivers conflict forecast data to the UN FAO for humanitarian decision-making,
- combines statistical analysis (posterior distributions, HDI, MAP) with production API serving,
- bridges multiple subsystems (Appwrite storage, geographic aggregation, caching, authentication),
- and must remain trustworthy over time as forecast methodology and data formats evolve.
...
- **`PredictionStoreManager`** -- Prediction-specific CRUD layer over `AppWriteFileManager` with metadata filtering, file versioning (latest-file resolution), and category-based retrieval (forecast vs. historical). Enforces prediction metadata schema via `PredictionMetadata`.

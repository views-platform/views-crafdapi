...
def test_sla_default_is_45_days(monkeypatch):
    monkeypatch.delenv("FAOAPI_FORECAST_FRESHNESS_SLA_DAYS", raising=False)
    assert freshness_sla_days() == 45.0


def test_sla_env_override_and_invalid_fallback(monkeypatch):
    monkeypatch.setenv("FAOAPI_FORECAST_FRESHNESS_SLA_DAYS", "7")
    assert freshness_sla_days() == 7.0
    monkeypatch.setenv("FAOAPI_FORECAST_FRESHNESS_SLA_DAYS", "banana")
    assert freshness_sla_days() == 45.0

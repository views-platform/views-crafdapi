...
It was extracted from `FAOApiManager._get_latest_dataframe` (a 205-LOC method) so
the data pipeline has one reason to change (the fetch/cache strategy), separate
from the HTTP surface (routing) and lifecycle that remain in `FAOApiManager`.

It is a **composed collaborator**, not a subclass: the caches it mutates are owned
by `FAOApiManager` and injected by reference (so routes, lifecycle clears, and
...
from views_faoapi.managers import freshness, selection
...
class DatasetService:
    ...
    def __init__(
        ...
        *,
        ...
        # Injected by reference — owned by FAOApiManager, shared with routes/lifecycle.

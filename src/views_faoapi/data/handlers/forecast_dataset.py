...
from views_faoapi.forecast.serialize.json_contract import (
...
from views_faoapi.data.handlers.grid_dataset import _GridDataset
...
class ForecastDataset(_GridDataset):
    ...
    turn a loaded forecast artifact into served HDI/MAP. ``FAO_PGMDataset`` is retained as an
    alias for back-compatibility (disk-cache pickles, ``isinstance``, notebooks).
    """
...
# Back-compat alias (Phase 4a, #112): the leaf was renamed FAO_PGMDataset -> ForecastDataset.
# Retained so existing disk-cache pickles (class path handlers.FAO_PGMDataset), isinstance
# checks, the ~18 test modules, and notebooks/geo_meta.ipynb keep resolving. The __init__
# signature is unchanged, so the disk-cache schema version (derived from it) is unchanged.
FAO_PGMDataset = ForecastDataset

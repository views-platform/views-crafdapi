"""Run the published views-frames conformance contracts on real faoapi frames (#91).

faoapi is a cross-repo consumer of the frozen views-frames leaf. Asserting the leaf's own
published contracts — the frame contract, the summarizer contract, and the cross-level
alignment law — on **real faoapi-built frames and the injected GAUL mapping** proves the full
...
def assert_frame(frame) -> None:
    """Assert the leaf frame contract + the summarizer contract on a faoapi-built frame."""
    assert_frame_contract(frame)
    assert_summarizer_contract(frame)


def assert_cross_level_law(index, mapping, target_level) -> None:
    """Assert the cross-level alignment law for faoapi's injected `(time, unit) -> target`

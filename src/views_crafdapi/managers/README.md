...
```python
manager = CrafdApiManager(api_key="...", port=8000, workers=4)
manager.run()
```
...
```bash
uvicorn views_crafdapi.managers.api:app --workers 4
```
...
logger = logging_module.get_logger("views_crafdapi.api")
...
```python
_manager_cache: Dict[str, Tuple[CrafdApiManager, float]]
```
...
```python
_dataframe_cache: Dict[str, Tuple[ForecastDataset, float]]
```
...
uvicorn views_crafdapi.managers.api:app --host 0.0.0.0 --port 8000 --workers 4
```
